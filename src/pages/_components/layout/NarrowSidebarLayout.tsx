...
export type NarrowSidebarLayoutProps = {
  children: React.ReactNode
}

export function NarrowSidebarLayout({ children }: NarrowSidebarLayoutProps) {
                ...
                return (
                  <a
                    ...
                    className={classNames(
                      isCurrent ? 'bg-indigo-800 text-white' : 'text-indigo-100 hover:bg-indigo-800 hover:text-white',
                      'group w-full p-3 rounded-md flex flex-col items-center text-xs font-medium'
                    )}
                    aria-current={isCurrent ? 'page' : undefined}>
                    <item.icon
                      className={classNames(isCurrent ? 'text-white' : 'text-indigo-300 group-hover:text-white', 'h-6 w-6')}
                      aria-hidden='true'
                    />
                        ...
                        {sidebarNavigation.map((item) => {
                          ...
                          return (
                            <a
                              ...
                              <item.icon
                                className={classNames(
                                  isCurrent ? 'text-white' : 'text-indigo-300 group-hover:text-white',
                                  'mr-3 h-6 w-6'
                                )}
                                aria-hidden='true'
                              />
                  ...
                  <Menu as='div' className='relative flex-shrink-0'>
                    ...
                    <Transition
                      ...
                      <Menu.Items className='origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none'>
                        {/* <Menu.Item>
                          {({ active }) => (
                            <a href={`/personnes/${session.userId}`}>
                              <div
                                className={classNames(
                                  active ? 'bg-gray-100' : '',
